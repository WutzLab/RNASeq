...
# DATAPATH needs to be set to load analysis data from and write output files
# CHIP_DATA_PATH needs to be set to a folder containing H3K27me3 ChIPseq data for correlation analysis with gene expression
...
class XpressionAnalysis(object):
    ...
    def genes_chromosomal_distribution(self, gene_list):
        """
        Takes a list of gene name strings and returns a sorted list of tuples with the first item the chromosome mame from gene annotation in
        self.genes dictionary and the second item being the number of genes for the chromosome. Sorting is in descending order of the number
        of genes for each chromosome.
        """
        ct = collections.Counter([self.genes[g]["chrom"] for g in gene_list])
        return sorted(ct.items(), key=lambda x: x[1], reverse=True)
...
LOAD_ANALYSIS = True
DATAPATH = "../RNAseq/" # path to load analysis from and write output files
...
if LOAD_ANALYSIS:
    analysisProject = XpressionAnalysis.from_csv(DATAPATH + "analysis_data")
else:
...
CHIP_DATA_PATH= "../ChIPseq/Programs/" # path to load ChIPseq data from for correlation of H3K27me3 with gene expression
...
def HeizMappe(analysis, column_order=None, LinReg=True, chrom="chr1", colorMap=None, genotype_sepLine_color=None, filename=None):
    ...
    ax1.set_title("chrX", loc='left', y=1.08)
    ax1.title.set_fontsize(20)
    # for matplotlib version 1:
    # seaborn.heatmap(zmapXnorm, cmap=colorMap, xticklabels=False, yticklabels=False, ax=ax1, cbar=False))
    seaborn.heatmap(zmapX, cmap=colorMap, vmin=-5, vmax=5, center=0, xticklabels=True, yticklabels=False, ax=ax1, cbar=False)
    ax1.xaxis.set_tick_params(labeltop='on', labelbottom='off')
    ax1.tick_params(axis="x", which="both", length=0) # for matplotlib version 2 to hide ticks at bottom
    ...
    ax2.title.set_fontsize(20)
    # for matplotlib version 1:
    # seaborn.heatmap(zmapXnorm, cmap=colorMap, xticklabels=False, yticklabels=False, ax=ax2, cbar=False))
    seaborn.heatmap(zmapA, cmap=colorMap, vmin=-5, vmax=5, center=0, xticklabels=True, yticklabels=False, ax=ax2, cbar=False)
    ax2.xaxis.set_tick_params(labeltop='on', labelbottom='off')
    ax2.tick_params(axis="x", which="both", length=0) # for matplotlib version 2
    ...
    ax3.set_title("chrX normalized Xs", loc='left', y=1.08)
    ax3.title.set_fontsize(20)
    seaborn.heatmap(zmapXnorm, cmap=colorMap, vmin=-5, vmax=5, center=0, xticklabels=True, yticklabels=False, ax=ax3, cbar=True, cbar_ax=ax4)
    # for matplotlib version 1:
    # seaborn.heatmap(zmapXnorm, cmap=colorMap, xticklabels=False, yticklabels=False, ax=ax3, cbar=True, cbar_ax=ax4)
    ax3.xaxis.set_tick_params(labeltop='on', labelbottom='off')
    ax3.tick_params(axis="x", which="both", length=0) # for matplotlib version 2
...
def getBenjaminiHochbergThreshold(pvals, alpha=0.05, quiet=True):
    """
    This function calculates the thresholds for p-values in scenarious of multiple testing.
    All results with unadjusted p-values lower or equal to the BH threshold are considered
    significant. If p-value exceeds the threshold the results can be rejected. An advantage
    over the Bonferroni correction of p-values for multiple testing is that p-values are
    maintained. A second advantage is a substantially improved sensitivity for large number
    of comparisons such as RNAseq gene lists.

    pvals (list)  ... list of unadjusted p-values
    alpha (float) ... desired FDR (default 0.05)

    Returns the BH threshold value as a floating point number.
    The implementation is adapted from Thadryan J. Sweeney' excellent blog post at:
    https://thadryanjs.github.io/posts/2022-12-17-multiple-testing-correction/
    """
    ...
    for i in range(num):
        rank = float(i+1)
        crit = (rank/num)*alpha
        critical_vals.append(crit)
    df_res = pd.DataFrame({"pvalue": sorted_pvals, "critical_value": critical_vals})
    df_res_p_less_critical = df_res.query("pvalue <= critical_value")
    df_res_p_less_critival = df_res_p_less_critical.sort_values(by = "pvalue", ascending = False)
    if len(df_res_p_less_critical) == 0:
        if not quiet:
            print(num, "p-values tested ( min =", min(pvals), "max =", max(pvals), "), but none is significant")
        BH_threshold = 0       # none of the p-values was lower than critical therefore all results are rejected
    else:                      # BH threshold is the larges p-value that was smaller than its critical value
        BH_threshold = df_res_p_less_critical["pvalue"].max()
        if not quiet:
            print(num, "p-values used ( min =", min(pvals), "max =", max(pvals), ") to determine Benjamini-Hochberg Threshold =", BH_threshold)
    return BH_threshold


def differentialGeneExpressionAnalysis(analysis, refGrp, sbjGrp, fold_change_threshold=2.0, min_any_TPM=0.1, min_sum_TPM=10, p_value_cutoff=0.01, Bonferroni=False, BenjaminiHochbergThreshold=False, chrom=None, normalize=None, filename=None, ax=None, show_figure=True, show_Xist=False, quiet=True):
    """
    Plots the fold change between "Dox" and "Ctrl" from read or TPM counts for a list of genes for the genotypes indicated.
    ...
    The function presumes that samples, and conditions are initialized as global variables.
    Bonferroni (Bool) : If a p-value correction for multiple testing is desired it can be calculated using the Bonferroni method
                        as p-value * number of genes in DF without NaNs truncated to be less or equal to 1 
                        Note that the p_value_cutoff will need to be set accordingly to a suitable family wise error eg 0.1 or 0.05
                        Both measurement (technical) and gene expression (biological) variability increase p-values. Therefore,
                        careful weighting of the statistical methods can improve chances of discovery. Although, statistical
                        rigor will prevent high false positive rates in any case, regulators might have low or variable expression
                        and although likely the most interesting genes and are among the first to be eliminated by statistics.
                        Therefore, it is encouraged to use p-values that are not mathematically corrected and inspect the list of
                        differentially expressed genes through the eye of a biologist for identifying such potential regulators,
                        (i.e. publish the list with corrected p-values and your discovery in separate manuscripts :-).
    BenjaminiHochbergThreshold (bool) : If True a p-value threshold will be calculated using the Benjamini-Hochberg method to obtain
                        a cutoff. Genes with p-values at or below the threshold will be considered significant, others rejected.
                        Note that if BenjaminiHochbergThreshold is selected no p-value correction will be applied even if
                        Benferroni is set True. The BenjaminiHochbergThreshold method is suitable for larger number of tests
                        including RNAseq datasets, where the loss of sensitivity from a Bonferroni correction would be problematic.
    """
    if type(analysis) == pd.DataFrame:
        analysisDF=analysis
    else:
    ...
    # perform t-test for independent samples ref-gt
    ...
    filteredDF=filteredDF[filteredDF[refGrp + sbjGrp].ge(min_any_TPM).all(axis=1)] # drop genes with lower than min TPM in any of the gts
    filteredDF=filteredDF[filteredDF[refGrp + sbjGrp].sum(axis=1) >= min_sum_TPM]  # drop genes with lower sum of TPM in all genotypes
    cutoff = p_value_cutoff
    if BenjaminiHochbergThreshold:     #--------------------------------------------------- Benjamini Hochberg added here ----------
        cutoff = getBenjaminiHochbergThreshold([e for e in filteredDF["p-value"]], alpha=p_value_cutoff, quiet=quiet)
    elif Bonferroni:
        numTests=len(filteredDF.index) #--------------------------------------------------- Bonferroni added here ------------------
        filteredDF["p-value"]=filteredDF["p-value"] * numTests
    ax.plot(filteredDF["fold"], 1/filteredDF["p-value"], ".", color="gray", alpha=0.5)
    filteredDF=filteredDF.sort_values(by="fold", ascending=False)
    upDF=filteredDF[ filteredDF["fold"]>=float(fold_change_threshold) ]    
    upDF=upDF[ upDF["p-value"]<=cutoff ]
    downDF=filteredDF[ filteredDF["fold"]<=1.0/float(fold_change_threshold) ]
    downDF=downDF[ downDF["p-value"]<=cutoff ]
    if not (filename is None):
        analysisDF.to_csv(DATAPATH+filename+".csv")
    upGenesList=list(upDF.index)
    ax.plot(upDF["fold"], 1/upDF["p-value"], ".", color="darkcyan")
    ...
    ax.plot(downDF["fold"], 1/downDF["p-value"], ".", color="firebrick")
    if not (fig is None):
        fig.tight_layout()
    if show_figure:
        plt.show()
...
def difGeneExprAllGenotypes(analysis, genotypes, filename=None, fold_change_threshold=2.0, p_value_cutoff=0.01, Bonferroni=False, BenjaminiHochbergThreshold=False, quiet=True):
    difRegGenes={}
    if len(genotypes)<2:
        if printVerboseLog:
    ...
    for gt in genotypes[1:]:
        with plt.style.context('seaborn-white'):
            ...
            difRegGenes[gt]=dict()
            difRegGenes[gt]["up"], difRegGenes[gt]["down"]=differentialGeneExpressionAnalysis(analysis, samples[refGt]["Ctrl"], samples[gt]["Ctrl"],
                                                                                              filename=None, fold_change_threshold=fold_change_threshold,
                                                                                              p_value_cutoff=p_value_cutoff, Bonferroni=Bonferroni,
                                                                                              BenjaminiHochbergThreshold=BenjaminiHochbergThreshold, ax=ax,
                                                                                              show_figure=False, quiet=quiet)
        panel_col+=1
        if panel_col>=2:
            panel_col=0
            panel_row+=1
...
# drg=difGeneExprAllGenotypes(tpmDF, ["WT","Ubn1","Ubn2","Hira","Ubn1/2"], fold_change_threshold=2, BenjaminiHochbergThreshold=True, p_value_cutoff=0.1, quiet=False)
# analysisProject.genes_chromosomal_distribution(drg["Ubn1/2"]["up"])
