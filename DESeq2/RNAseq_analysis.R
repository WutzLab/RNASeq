...
samplename <- c()
for (i in 1:length(fls)) {
   samplename <- c(samplename, concatstr(genotype[i], "_", as.character(clonenumber[i])))
}

cat("genotype\tclone number\tcondition\tcell line\tsample name\tfilename\n")
for (i in 1:length(fls)) {
   cat(concatstr(genotype[i], concatstr("#",as.character(clonenumber[i])), condition[i], celline[i], samplename[i], fls[i], sep='\t\t'))
   cat("\n")
}
...
ddsHTSeq <- DESeqDataSetFromHTSeqCount(sampleTable = sampleTable,
                                       directory = directory,
                                       design= ~ genotype)
ddsHTSeq$genotype <- factor(ddsHTSeq$genotype, levels = c("WT", "Ubn1", "Ubn2", "Ubn12", "Hira"))
...
for(gt in c("Ubn1","Ubn2","Ubn12","Hira")) {
   print(concatstr("Results for ",gt," vs WT"))
   res05 <- results(ddsHTSeq, contrast=c("genotype",gt,"WT"), alpha=0.05)
   resOrdered <- res05[order(res05$pvalue),]
   print(concatstr("./",gt,"_vs_WT_results.csv"))
   write.csv(as.data.frame(resOrdered), 
             file=concatstr("./",gt,"_vs_WT_results.csv"))
   resSig <- subset(resOrdered, padj <= 0.05)
   print(concatstr("./",gt,"_vs_WT_results05.csv"))
   write.csv(as.data.frame(resSig), 
             file=concatstr("./",gt,"_vs_WT_results05.csv"))
   resultsNames(res05)
   summary(res05)
